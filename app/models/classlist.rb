class Classlist < ApplicationRecord
  ...
  def enrollment_was_edited
    ...
    if old_student && old_section
      remove_subject_from(old_student, old_section.subject)
    end
  ...
  def add_subject_to(a_student, a_subject)
    ...
    a_student.subject_count += 1
    a_student.number_of_units += a_subject.number_of_units
    a_student.tuition_fee +=
      a_subject.number_of_units * a_subject.per_unit_rate
